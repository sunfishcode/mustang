...
//!
//! Currently everything is enabled except for the test that uses threads,
//! which is marked with `#[cfg(feature = "threads")]`.
...
fn symlink_hard_link() {
    ...
    assert_eq!(
        check!(fs::read_link(tmpdir.join("symlink"))),
        Path::new("file")
    );
    assert_eq!(
        check!(fs::read_link(tmpdir.join("hard_link"))),
        Path::new("file")
    );
